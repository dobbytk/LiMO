...
def extract_checkstyle_modules(xml_path):
    tree = ET.parse(xml_path)
    root = tree.getroot()

    check_modules = []

    def recurse_modules(element):
        for module in element.findall('module'):
            name = module.attrib.get('name')
            if name not in ['Checker', 'TreeWalker']:
                check_modules.append(name)
            recurse_modules(module)
    recurse_modules(root)
    return check_modules
xml_file_path = "/KSY_copy/train_java/google_checks.xml"
xml_file_path_sun = "/KSY_copy/train_java/sun_checks.xml" 
error_codes = extract_checkstyle_modules(xml_file_path)
error_codes_sun = extract_checkstyle_modules(xml_file_path_sun)

error_codes = list(set(error_codes))
error_codes_sun = list(set(error_codes_sun))
error_codes += error_codes_sun
error_codes = list(set(error_codes)) # 110개

    
def analyze_checkstyle_output(eval_result: str, error_codes: list) -> Counter:
    analysis = [0]*len(error_codes)
    error_pattern = re.compile(r"Main\.java:(\d+):\d+:.+\[([A-Za-z0-9]+)\]")
...
def analyze_checkstyle_output_line(eval_result: str, total_lines: int, error_codes: list):
    error_pattern = re.compile(r"Main\.java:(\d+):\d+:.+\[([A-Za-z0-9]+)\]")
    errors = error_pattern.findall(eval_result)
    ...
    def test(self, test_dataloader, content_level_eval=False, prediction_method="most_common"):
        ...
        for step, inputs in enumerate(
            ...
            with torch.no_grad():
                ...
                total_logits.extend(logits.cpu())
                total_probs.extend(probabilities)
        ...
        return sent_result, content_result, {'text': texts, 'pred': pred_labels, 'true': true_labels, 'problem_id':problem_ids, 'user_id':user_ids, 'line_count':line_counts, 'probs':total_probs}
    ...
    def _get_precision_recall_acc_f1(self, true_labels, pred_labels, pred_probs=None, pos_label: int = 1) -> Dict[str, Any]:
        ...
        if pred_probs is not None:
            ...
            def eval_at(thr: float, tag: str) -> Dict[str, Any]:
                ...
                return {"thr": float(thr), "accuracy": acc_, "macro_f1": mF1_, "binary_f1": bF1_, "precision": pr_, "recall": rc_, "cm": cm_, "fpr":fpr, "tpr":tpr}
...
def split_dataset(data_path, dataset, seed=42, test_size=0.2, val_size=0.1):
    # 1) Load full set
    with open(os.path.join(data_path, f"codenet(java)_{dataset}_features.jsonl"), "r", encoding="utf-8") as f:
    ...
    for i, sample in enumerate(full_train_set):
        ...
        if 'line' in dataset:
            ...
            ccfeature_line = analyze_checkstyle_output_line(sample.get('eval', ''), n_lines, error_codes)
            sample['ccfeature'] = ccfeature_line
        else:
            sample['ccfeature'] = analyze_checkstyle_output(sample.get('eval', ''), error_codes)
...
def parse_args():
    ...
    # parser.add_argument('--scenario_human', type=str)
...
if __name__ == "__main__":
    ...
    df_ = split_dataset(data_path=args.data_path, dataset=args.dataset, seed=args.seed)  # [train, val, test]
    ...
    ckpt_name = f'ckpt/java/{args.ckpt_name}_best_f1.pt'

    trainer = SupervisedTrainer(datas, classifier, en_labels, id2label, args)
    trainer.writer = SummaryWriter(log_dir=f"runs/java/{args.ckpt_name}")
    ...
    with open(f'train_java/result/new/experiment_results_{args.ckpt_name}.json', 'w') as file:
