...
class MultiModalConcatLineFocalBMESBinaryClassifier(nn.Module):
    def __init__(self, id2labels, seq_len, intermediate_size = 512, num_layers=2, dropout_rate=0.1, alpha=0.5): # encoder_checkpoint_path=''
        ...
        combined_embedding_size = embedding_size + feature_embedding_size
        # =======KTH========
        # self.encoder_checkpoint_path = encoder_checkpoint_path
        # self.feature_encoder = TransformerClassifier.load_from_checkpoint(self.encoder_checkpoint_path)
        self.feature_encoder = TransformerClassifier(input_dim=289, hidden_dim=256, output_dim=1, n_heads=4, n_layers=2)
    ...
    def forward(self, x, labels, ccfeature): 
        # print(f"[INPUT] x: {x.shape}, labels: {labels.shape}, ccfeature: {ccfeature.shape}")
        mask = labels.gt(-1)
        padding_mask = ~mask
        
        x = x.transpose(1, 2)
        # print(f"[CONV INPUT] shape: {x.shape} VALUE: {x}")
        out1 = self.conv_feat_extract(x[:, 0:1, :])  
        # print(f"[CONV OUT1] shape: {out1.shape} VALUE: {out1}")
        ...
        # print(f"[CONV OUT] shape: {out.shape} VALUE: {out}")
        ...
        # print(f"[BEFORE ENCODER] outputs shape: {outputs.shape}, padding_mask: {padding_mask.shape}, VALUE: {outputs}")
        # === KTH ===
        # with torch.no_grad():
        #     feature_embedding = self.feature_encoder.get_feature_embedding(ccfeature, padding_mask)
        # print(f"[INPUT] CCFEATURE: {ccfeature.shape} VALUE: {ccfeature}")
        feature_embedding = self.feature_encoder.get_feature_embedding(ccfeature, padding_mask)
        # print(f"[OUTPUT_CC ENCODER] feature_embedding: {feature_embedding.shape} VALUE: {feature_embedding}")
        ...
        combined_out = torch.cat((outputs, feature_embedding), dim=2)  # [32, 1024, 256+256] 16]
        
        # print(f"[COMBINED] before reduce: {combined_out.shape} VALUE: {combined_out}")
        combined_out = self.reduce_dim_layer(combined_out)
        # print(f"[COMBINED] after reduce: {combined_out.shape} VALUE: {combined_out}")
        ...
        logits = self.classifier(dropout_outputs) # Classification Layer
        ...
        if self.training:
            ...
            # print(f"[LOSS_CLASS] shape: {loss_class.shape} VALUE: {loss_class}")
            
            # Binary classification 부분
            logits_softmax = torch.softmax(logits, dim=-1)
            # print(f"LOGITS_SOFTMAX: {logits_softmax}, VALUE: {logits_softmax}")
            ai_probs = torch.sum(logits_softmax[:,:,4:8], dim=-1)
            # print(f"AI_PROBS: {ai_probs.shape} VALUE: {ai_probs}")
            binary_logits = ai_probs # pred_label
            
            # Binary labels 생성: 0 for human (0-3), 1 for AI (4-7)
            binary_labels = (labels >= 4).float() # true label
            loss_binary = self.compute_masked_loss(binary_logits, binary_labels, mask.float())
            
            loss = self.alpha * loss_class +  (1-self.alpha) * loss_binary
...
class NoccfLineFocalBMESBinaryClassifier(nn.Module):
    def __init__(self, id2labels, seq_len, intermediate_size = 512, num_layers=2, dropout_rate=0.1, alpha=0.5):
        super(NoccfLineFocalBMESBinaryClassifier, self).__init__()
        # feature_enc_layers = [(512, 10, 5)] + [(512, 3, 2)] * 4 + [(512,2,2)] + [(512,2,2)]
        feature_enc_layers = [(64, 5, 1)] + [(128, 3, 1)] * 3 + [(64, 3, 1)]
        self.conv = ConvFeatureExtractionModel(
            conv_layers=feature_enc_layers,
