...
from model_4_ablation import MultiModalConcatLineFocalBMESBinaryClassifier, NoccfLineFocalBMESBinaryClassifier
...
with open("/KSY_copy/train_cpp/cpp_errorcode.txt") as f:
    error_codes = [error.replace('\n', '') for error in f.readlines()]
    
def analyze_cpplint_output(eval_result: str, error_codes: list) -> Counter:
    analysis = [0]*len(error_codes)
    # 정규 표현식을 사용하여 에러 코드 추출
    error_pattern = re.compile(r'\[([a-z0-9_]+\/[a-z0-9_]+)\]')
...
def analyze_cpplint_outputs(eval_results: list, error_codes: list) -> Counter:
    analysis = {}
    for e in error_codes:
        analysis[e] = [0]*len(eval_results)
    # 정규 표현식을 사용하여 에러 코드 추출
    error_pattern = re.compile(r":(\d+):.*?\[([a-z]+\/[a-z0-9_]+)\]")
    for idx, eval_result in enumerate(eval_results):
        ...
        for error_code, count in error_counts.items():
            analysis[error_code][idx] = count
            #print(f"{error_code}: {count} occurrences")
            
    return analysis


def analyze_cpplint_output_line(eval_result: str, total_lines: int, error_codes: list):
    # 줄 번호와 에러 코드를 추출하는 정규 표현식
    error_pattern = re.compile(r":(\d+):.*?\[([a-z]+\/[a-z0-9_]+)\]")
    errors = error_pattern.findall(eval_result)
    
    # 각 줄별로 에러 코드를 카운트하기 위한 딕셔너리
    line_error_counts = defaultdict(Counter)
    # 각 줄별 에러 코드를 카운트
    for line, code in errors:
        line_error_counts[int(line)][code] += 1

    # 전체 라인별 분석 결과 리스트 초기화
...
class SupervisedTrainer:
    ...
    def train(self, ckpt_name='linear_en.pt', prediction_method="most_common"):
        ...
        if self.best_model_path:
            ...
            self.model.load_state_dict(torch.load(self.best_model_path).state_dict())
            self.model.to(self.device)
    ...
    def test(self, test_dataloader, content_level_eval=False, prediction_method="most_common"):
        ...
        return sent_result, content_result, {'text':texts,'pred':pred_labels, 'true':true_labels}
    ...
    def sent_level_eval(self, texts, true_labels, pred_labels, pred_probs, prediction_method='most_common'):
        ...
        for text, true_label, pred_label, pred_prob in zip(texts, true_labels, pred_labels_threshold, pred_probs):
            ...
            for true_label_idx in range(len(true_label)):
                if true_label_idx >= len(sents):
                    print(f"[WARNING] Label index {true_label_idx} exceeds sentence count {len(sents)}. Skipping.")
                    continue
        ...
        for text, true_label, pred_label, pred_prob in zip(texts, true_labels, pred_labels_threshold, pred_probs):
            ...
            for true_label_idx in range(len(true_label)):
                if sents[true_label_idx] == '' or sents[true_label_idx].isspace():  # 빈 문장일 경우 처리하지 않음
                    continue
                true_word_label = self.id2label[true_label[true_label_idx]]
                pred_word_label = self.id2label[pred_label[true_label_idx]]
                
                true_word_labels.append(true_word_label.split('-')[-1])
                pred_word_prob = pred_prob[true_label_idx, 4:8].sum()
                pred_word_probs.append(pred_word_prob.item())
                pred_word_labels.append(pred_word_label.split('-')[-1])
            
            true_sent_label, _ = self.get_sent_label(text, true_label)
            pred_sent_label, pred_sent_prob = self.get_sent_label(text, pred_label, pred_prob, threshold)
            if len(true_sent_label) != len(pred_sent_prob):
                print("sent lable size mismatched")
                continue
            true_sent_labels.extend(true_sent_label)
            pred_sent_labels.extend(pred_sent_label)
            pred_sent_probs.extend(pred_sent_prob)
            
        
        true_sent_labels = [self.en_labels[label] for label in true_sent_labels]
        pred_sent_labels = [self.en_labels[label] for label in pred_sent_labels]
        true_word_labels = [self.en_labels[label] for label in true_word_labels]
        pred_word_labels = [self.en_labels[label] for label in pred_word_labels]
        sent_result = self._get_precision_recall_acc_f1(true_sent_labels, pred_sent_labels, pred_sent_probs)
        
        print("*" * 8, "Word Level Evalation", "*" * 8)
        word_result = self._get_precision_recall_acc_f1(true_word_labels, pred_word_labels, pred_word_probs)
        return word_result, sent_result
        

    def get_sent_label(self, text, label, prob=None, machine_threshold=None):
        import nltk
        sents = text.split('\n')
        ...
        for sent in sents:
            ...
            try:
                ...
                sent_label = most_common_tag[0]
            except:
                continue
            sent_labels.append(sent_label)
            
            if prob != None:
                tags = prob[start_word_idx:end_word_idx, :]
                machine_prob = tags[:, 4:8].sum(dim=1)
                sent_prob = torch.mean(machine_prob, dim=0)
                sent_probs.append(sent_prob.item())
        
        if len(sent_labels) == 0:
            print("empty sent label list")
        return sent_labels, sent_probs
    """
...
def split_dataset(data_path, dataset, seed=42, test_size=0.2, val_size=0.1):
    ...
    with open(os.path.join(data_path, f"codenet(cpp)_{dataset}_features.jsonl"), "r", encoding="utf-8") as f:
    ...
    for i, sample in enumerate(full_train_set):
        ...
        if 'line' in dataset:
            ...
            ccfeature_line = analyze_cpplint_output_line(sample.get('eval', ''), n_lines, error_codes)
            sample['ccfeature'] = ccfeature_line
        else:
            sample['ccfeature'] = analyze_cpplint_output(sample.get('eval', ''), error_codes)
    ...
    train_full_idx, test_idx = next(gss.split(np.zeros(len(full_train_set)), labels, groups=groups))
...
if __name__ == "__main__":
    ...
    df_ = split_dataset(data_path=args.data_path, dataset=args.dataset, seed=args.seed)
    ...
    if args.method == 'focalbmesbinary_embedconcat_transformer256':
        if 'gemini_hybrid' in args.dataset:
            classifier = MultiModalConcatLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len,
                                                encoder_checkpoint_path=f'train_cpp/ccf_model/cpp/Transformer_256_seq_15_60_gemini/version_{fold}/checkpoints/Transformer_256_seq_15_60_gemini_Fold{fold}_best-checkpoint.ckpt', alpha=args.alpha)
        elif 'gpt4_hybrid' in args.dataset:
            classifier = MultiModalConcatLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len,
                                                encoder_checkpoint_path=f'train_cpp/ccf_model/cpp/Transformer_256_seq_15_60_gpt4/version_{fold}/checkpoints/Transformer_256_seq_15_60_gpt4_Fold{fold}_best-checkpoint.ckpt', alpha=args.alpha)
        elif 'gemini' in args.dataset:
            classifier = MultiModalConcatLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len,
                                                encoder_checkpoint_path=f'train_cpp/ccf_model/cpp/Transformer_256_doclineloss_document_seq_30_60_gemini_2e-5/version_{fold}/checkpoints/Transformer_256_doclineloss_document_seq_30_60_gemini_2e-5_Fold{fold}_best-checkpoint.ckpt', alpha=args.alpha)
        elif 'gpt4' in args.dataset:
            classifier = MultiModalConcatLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len,
                                                encoder_checkpoint_path=f'train_cpp/ccf_model/cpp/Transformer_256_doclineloss_document_seq_30_60_gpt4_1e-4/version_{fold}/checkpoints/Transformer_256_doclineloss_document_seq_30_60_gpt4_1e-4_Fold{fold}_best-checkpoint.ckpt', alpha=args.alpha)
    elif args.method == 'noccf_LineFocalBMESBinaryClassifier':
        if args.testbed == 'toplevel':
            if 'hybrid' in args.dataset:
                if 'gemini' in args.dataset:
                    classifier = NoccfLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len, alpha=args.alpha)
                elif 'gpt4' in args.dataset:
                    classifier = NoccfLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len, alpha=args.alpha)
                ...
                classifier = NoccfLineFocalBMESBinaryClassifier(id2labels=id2label, seq_len=args.seq_len, alpha=args.alpha)
    ckpt_name = f'ckpt/cpp/{args.ckpt_name}_best_f1.pt'
    trainer = SupervisedTrainer(datas, classifier, en_labels, id2label, args)
    
    trainer.writer = SummaryWriter(log_dir=f"runs_3fold/cpp/{args.ckpt_name}")
    ...
    if args.do_test:
        ...
        if 'hybrid' in args.dataset or 'revised' in args.dataset:
            #test_word_result, test_sent_result, _ , test_raw_results = trainer.test(datas.test_dataloader, content_level_eval=False, prediction_method=prediction_method)
            ...
            #test_word_result, test_sent_result, test_content_result, test_raw_results = trainer.test(datas.test_dataloader, content_level_eval=True, prediction_method=prediction_method)
    ...
    with open(f'./train_cpp/result/experiment_results_{args.ckpt_name}.json', 'w') as file:
